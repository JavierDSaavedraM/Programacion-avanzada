...
int minOperations(const string& s, int& l1, int& r1, int& l2, int& r2, int& l3, int& r3) {
    ...
    if (s_size == 0){
        throw invalid_argument("La cadena no puede estar vacia");
    } 
    for (char c:s){
        if (c < 'a' || c > 'z'){
            throw invalid_argument("La cadena debe contener solo minusculas (a-z)");
        }
    }
...
int main() {
    ...
    int min_operations;
    try {
        min_operations = minOperations(s, l1, r1, l2, r2, l3, r3);
