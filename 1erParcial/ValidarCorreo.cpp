...
bool Validar_Usuario(string usuario){
    if (usuario.size()<1){
        cout << "El usuario '"<< usuario << "' cuenta con muy pocos caracteres, incorrecto" << endl;
        return false;
    }
    
    if (usuario.size() > 20){
        cout << "El usuario '"<< usuario << "' cuenta con muchos caracteres, incorrecto" << endl;
        return false;
    }
    ...
    {  
        cout << "Error, el usuario '"<< usuario << "' contiene el siguiente caracter, no es valido " << match.str() << endl;
        return false;
    }
    pattern = regex(R"(^\d)");
    if (regex_search(usuario,match,pattern))
    {  
        cout << "Error, el usuario '"<< usuario << "' debe de comenzar con una letra minuscula" << endl;
        return false;
    }
    pattern = regex(R"(_$)");
    if (regex_search(usuario,match,pattern))
    {  
        cout << "Error, el usuario '"<< usuario << "' no puede terminar con un guion bajo" << endl;
        return false;
    }
    pattern = regex(R"(__)");
    if (regex_search(usuario,match,pattern))
    {  
        cout << "Error, el usuario '"<< usuario << "' no puede contener dos guiones bajos consecutivos" << endl;
        return false;
    }
...
bool Validar_Dominio(string dominio){
    ...
    {  
        cout << "Error, el dominio '"<< dominio << "' no puede contener letras mayusculas" << endl;
        return false;
    }
    ...
    if (regex_search(dominio,match,pattern))
    {  
        cout << "Error, el dominio '"<< dominio << "' no puede contener un caracter que no sea una letra minuscula, un numero o un punto" << endl;
        return false;
    }
    ...
    if (regex_search(dominio,match,pattern))
    {  
        cout << "Error, el dominio '"<< dominio << "' no puede contener mas de 1 punto" << endl;
        return false;
    }
...
int main(){
    ...
    for (int i = 0; i < candidatos1.size(); i++) {
        ...
        {
            correctos.push_back(candidatos1[i]);
        }
    }
    
    cout << "\nCorreos correctos: " << endl;
    for (int i = 0; i < correctos.size(); i++)
    {
        cout << correctos[i] << endl;
    }
